...
function CustomPracticeCard({ onClick }: { onClick: () => void }) {
  return (
    <Stack
      ...
      bg="bg.card"
      cursor="pointer"
      _hover={{ bg: 'green.50', borderColor: 'brand.500' }}
      onClick={onClick}
      role="button"
...
function TopicCard({ topic, passed, total, onClick }: TopicCardProps) {
  ...
  return (
    <Stack
      ...
      borderRadius="xl"
      borderWidth="2px"
      borderColor={allDone ? 'green.300' : 'gray.100'}
      bg={allDone ? 'green.50' : 'bg.card'}
      cursor="pointer"
      _hover={{ bg: allDone ? 'green.100' : 'green.50', borderColor: allDone ? 'green.400' : 'brand.500' }}
      onClick={onClick}
      role="button"
